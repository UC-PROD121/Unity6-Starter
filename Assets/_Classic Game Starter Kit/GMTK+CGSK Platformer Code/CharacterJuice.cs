...
public class CharacterJuice : MonoBehaviour {
    public InfoProperty info = new InfoProperty( "Juice Component is NOT Complete", "If you want to use this, you'll need to either add Animation and ParticleSystem components or avoid the null reference exceptions when this tries to call them." );
    ...
    private void TiltCharacter()
    {
        ...
        Vector3 targetRotVector = new Vector3(0, 0, Mathf.Lerp(-characterSettingsSO.maxTilt, characterSettingsSO.maxTilt, Mathf.InverseLerp(-1, 1, directionToTilt)));

        //And then rotate the character in that direction
        if (myAnimator != null) myAnimator.transform.rotation = Quaternion.RotateTowards(myAnimator.transform.rotation, Quaternion.Euler(-targetRotVector), characterSettingsSO.tiltSpeed * Time.deltaTime);
    }
    ...
    private void CheckForLanding()
    {
        if (!playerGrounded && jumpScript.onGround)
        {
            ...
            if (!landSqueezing && characterSettingsSO.landSqueezeMultiplier > 1)
            {
                StartCoroutine(JumpSqueeze( characterSettingsSO.landSquashSettings.x * characterSettingsSO.landSqueezeMultiplier, characterSettingsSO.landSquashSettings.y / characterSettingsSO.landSqueezeMultiplier, characterSettingsSO.landSquashSettings.z, characterSettingsSO.landDrop, false));
            }
    ...
    public void JumpEffects()
    {
        ...
        if (!jumpSqueezing && characterSettingsSO.jumpSqueezeMultiplier > 1)
        {
            StartCoroutine(JumpSqueeze( characterSettingsSO.jumpSquashSettings.x / characterSettingsSO.jumpSqueezeMultiplier, characterSettingsSO.jumpSquashSettings.y * characterSettingsSO.jumpSqueezeMultiplier, characterSettingsSO.jumpSquashSettings.z, 0, true));
